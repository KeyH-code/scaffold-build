...
## 总目标
产出与官方**逐文件 diff 内容一致**的 Maven 多模块工程骨架：根 pom + 3 子模块 pom + 启动类 + sky-server 包结构骨架；理解"父 pom 管版本、子模块只管依赖"的组织方式。
（验收细则见 check.md 核对 1：XML 顺序/空行差异放行；缺内容/写错必改。）
...
- **知识沉淀（Obsidian 笔记）**：触发条件两条，满足其一即记录进 `E:\deeplearning\dl\Java`：①知识点切换文件/代码类型时（如 pom.xml → .java）；②大阶段（roadmap 里程碑）完成时；**记录更新完成后才进入下一步**；笔记书写标准（官方术语主体 / FAQ 重点 / 结构模板等）见全局 learning-mission.md「知识沉淀」；
- **开场抽查**：新单元开始时，老师先随机抽查上一单元 2-3 个核心知识点（不提示不剧透），不过先补学（详见全局 mission.md「教学协作约定」）；
- **课程总验收（可选）**：全部课程结束后，用户可选择将全部知识点讲给老师、老师提问检验（反转课堂式，非强制，详见全局 mission.md「教学协作约定」）；
