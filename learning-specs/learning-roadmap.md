...
## 后续规划占位（建议，不设计计划）
- 单元 5 验收后、vibecoding 开始前，**建议**插入过渡单元补充学习——方向：MyBatis 进阶（动态 SQL + 分页）/ Redis 缓存 / AOP + 事务的 Spring 落地（依据：vibecoding 高频且与框架知识强绑定；学习者已有 SQL 与事务概念基础，Redis/AOP 为空白）
- 具体单元计划**届时另行设计**，本文件不预先编写
